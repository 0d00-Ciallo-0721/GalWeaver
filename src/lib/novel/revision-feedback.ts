...
export function buildRevisionDirectives(feedback: NovelRevisionFeedback | null | undefined): string {
  if (!feedback) return ""
  const sections: string[] = []
  const mustFix = Array.isArray(feedback.mustFix) ? feedback.mustFix : []
  const shouldImprove = Array.isArray(feedback.shouldImprove) ? feedback.shouldImprove : []
  const carryToNextChapter = Array.isArray(feedback.carryToNextChapter) ? feedback.carryToNextChapter : []

  if (mustFix.length > 0) {
    sections.push(i18n.t("novel.revisionFeedback.mustFix"))
    mustFix.forEach((item) => sections.push(`  - ${item}`))
  }

  if (shouldImprove.length > 0) {
    sections.push(i18n.t("novel.revisionFeedback.shouldImprove"))
    shouldImprove.forEach((item) => sections.push(`  - ${item}`))
  }

  if (carryToNextChapter.length > 0) {
    sections.push(i18n.t("novel.revisionFeedback.carryToNextChapter"))
    carryToNextChapter.forEach((item) => sections.push(`  - ${item}`))
  }
